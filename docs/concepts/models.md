...
有关认证配置文件轮换、冷却期以及它们如何与回退交互的内容，请参阅 [/concepts/model-failover](/concepts/model-failover)。
...
3. **提供商认证故障转移**在切换到下一个模型之前在提供商内部发生。
...
这发生在正常回复生成**之前**，因此消息可能感觉像是"没有响应"。解决方法是要么：
...
- 如果模型 ID 本身包含 `/`（OpenRouter 风格），您必须包含提供商前缀（示例：`/model openrouter/moonshotai/kimi-k2`）。
- 如果您省略提供商，OpenClaw 将输入视为别名或**默认提供商**的模型（仅在模型 ID 中没有 `/` 时有效）。
...
OAuth 状态始终显示（并包含在 `--json` 输出中）。如果配置的提供商没有凭据，`models status` 会打印**缺少认证**部分。
...
`openclaw models scan` 检查 OpenRouter 的**免费模型目录**，并可选择探测模型的工具和图片支持。
...
在 TTY 中运行时，您可以交互式选择回退。在非交互式模式下，传递 `--yes` 接受默认值。
